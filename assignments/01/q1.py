...
x = tf.random_normal([10,10], mean=15, stddev=2)
...
###############################################################################
value = tf.constant([5, 2, 3, 5, 10, 6, 2, 3, 4, 2, 1, 1, 0, 9])
x, _ = tf.unique(value)
print(sess.run(x))
...
x = tf.random_normal(shape=[300], mean=15, stddev=2)
y = tf.random_normal(shape=[300], mean=15, stddev=2)
average = tf.reduce_mean(x - y)

def reduce_mean_square():
    return tf.reduce_mean(tf.square(x - y))

def reduce_mean_abs():
    return tf.reduce_mean(tf.abs(x - y))

out = tf.cond(average < 0, reduce_mean_square, reduce_mean_abs)
print(sess.run(out))
